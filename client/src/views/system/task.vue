<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogTableVisible" title="任务详细信息">
        <el-table >
            <el-table-column property="task_name" label="任务名称" width="50">
                    <template scope="scope"><span>{{ scope.row.task_name }}</span></template>
            </el-table-column>
  ...
  methods: {
    ...
    handleFilter() {       //搜索
      const newData = this.taskList.filter(
        data =>
          !this.search ||
          data.task_name.toLowerCase().includes(this.search.toLowerCase())
      )
